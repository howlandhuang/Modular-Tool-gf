...
class PlotProcessor:
    ...
    def check_column_match(self, noise_type, fig_type):
        ...
        """
        logger.info(f"Checking data consistency for noise type: {noise_type}")
        shape = None
        for device_name, wafer_id, bias_id, df in self.dataframes:
            ...
            if self.freq is None:
                ...
                logger.debug(f"Reference frequency range set with {len(self.freq)} points")
            ...
            logger.debug(f"Column count for current file: {current_columns}")
    ...
    def _plot_data(self, noise_type, fig_type, save_name):
        ...
        try:
            ...
            if not self.config.debug_flag:
                output_path = f'{self.config.output_path}/{save_name}_{title.replace("/", "_")}.png'
